	...
	for config in configs:
		...
		for cn0_log in cn0_logs:
			for test_iter in range(20):
                ...
                y_dopp_train = np.array([x['delta_dopp'] for x in data_train])
                y_dopp_val = np.array([x['delta_dopp'] for x in data_val])
                y_tau_train = np.array([x['delta_tau'] for x in data_train])
                y_tau_val = np.array([x['delta_tau'] for x in data_val])
                

				# Define/ compile model
#				if sys.argv[1] == '40':
				model = ModelRegressor(shape=(X_train.shape[1], X_train.shape[2], X_train.shape[3]))
#				elif sys.argv[1] == '30':
